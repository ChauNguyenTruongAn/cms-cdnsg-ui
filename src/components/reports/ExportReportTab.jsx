...
export default function ExportReportTab() {
  ...
  const fetchReport = async () => {
    setLoading(true);
    try {
      const res = await receiptService.getExportReport(filters);
      setReportData(res.data || []);
      if (res.data?.length === 0)
        showToast("Không có dữ liệu trong khoảng thời gian này", "info");
    } catch (error) {
      showToast("Lỗi khi tải báo cáo xuất kho!", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportExcel = () => {
    ...
    reportData.forEach((day) => {
      day.details.forEach((item, idx) => {
        excelData.push({
          "NGÀY XUẤT": idx === 0 ? day.exportDate : "",
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "BaoCaoXuat");
    worksheet["!cols"] = [
      { wch: 15 },
      { wch: 20 },
      { wch: 25 },
    ...
    XLSX.writeFile(
      workbook,
      `Bao_Cao_Xuat_${filters.fromDate}_den_${filters.toDate}.xlsx`,
    );
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      {/* Filters */}
      <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 grid grid-cols-1 md:grid-cols-6 gap-4">
        <div>
          ...
          <input
            type="date"
            className="w-full p-2.5 rounded-lg border outline-none focus:border-amber-600"
        ...
        <div className="flex items-end space-x-2">
          <button
            onClick={fetchReport}
            disabled={loading}
            className="flex-1 bg-amber-600 text-white p-2.5 rounded-lg font-bold flex justify-center items-center hover:bg-amber-700"
          >
            {loading ? (
                  ...
                  >
                    {iIndex === 0 && (
                      ...
                      >
                        ...
                        <span className="text-xs font-bold text-amber-600 bg-amber-100 px-2 py-1 rounded-full mt-2 inline-block">
