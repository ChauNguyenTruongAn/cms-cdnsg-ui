...
import { X, Plus, Trash2, Loader2, ClipboardCheck } from "lucide-react";
import { uniformService } from "../../services/uniformService";
import { useToast } from "../../context/ToastContext";
import SearchableSelect from "../common/SearchableSelect";

export default function CreateUniformReceiptModal({
  isOpen,
  onClose,
  onSuccess,
}) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await uniformService.createReceipt(payload); //
      showToast("Cấp phát đồng phục thành công!");
      onSuccess();
      onClose();
    } catch (error) {
      showToast(
        error.response?.data?.message ||
          "Lỗi khi xuất kho (Có thể do thiếu tồn kho)!",
        "error",
      );
    } finally {
      setIsSaving(false);
